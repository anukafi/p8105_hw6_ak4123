...
nested_glm =
  ...
  group_by(city_state) %>% 
  nest() %>% 
  mutate(reg_city_state = map(data, ~lm(resolution ~ victim_age + victim_sex + victim_race, data = .x)),
         reg_city_state = map(reg_city_state, broom::tidy)) %>% 
  select(-data)

nested_glm = nested_glm %>% 
  unnest() %>% 
  janitor::clean_names() %>% 
  mutate(OR = exp(estimate),
